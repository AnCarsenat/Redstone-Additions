...
# Membership form, for the one-macro test in props/render.
...
# The same names as a list, because the raw properties line has to have them removed
# one at a time and a compound cannot be walked.
data modify storage ra:dh creative_only_names set value ["cooldown","transfer_rate","generation_rate","eu_use","tier","tier_level"]
