...
execute as @e[type=marker,tag=ra.custom_block.clock] at @s if block ~ ~ ~ #minecraft:air run tag @s add ra.broken
execute as @e[type=marker,tag=ra.broken,tag=ra.custom_block.clock] at @s run kill @e[type=item,nbt={Item:{id:"minecraft:daylight_detector"}},distance=..2,limit=1,sort=nearest]
