# Delayer delay: down by 5. Applies to blocks placed from now on.
scoreboard players set #n ra.set.tmp 20
execute if data storage ra:settings global.props."delayer"."delay" store result score #n ra.set.tmp run data get storage ra:settings global.props."delayer"."delay"
...
execute store result storage ra:settings global.props."delayer"."delay" int 1 run scoreboard players get #n ra.set.tmp
