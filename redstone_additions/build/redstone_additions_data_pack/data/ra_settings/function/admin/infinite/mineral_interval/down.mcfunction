# Mineral interval: down by 10. Applies to blocks placed from now on.
scoreboard players set #n ra.set.tmp 100
execute if data storage ra:settings global.props."mineral_generator"."cooldown" store result score #n ra.set.tmp run data get storage ra:settings global.props."mineral_generator"."cooldown"
...
execute store result storage ra:settings global.props."mineral_generator"."cooldown" int 1 run scoreboard players get #n ra.set.tmp
