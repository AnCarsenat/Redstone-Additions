# Liquid Drain interval: down by 5. Applies to blocks placed from now on.
scoreboard players set #n ra.set.tmp 20
execute if data storage ra:settings global.props."liquid_drain"."cooldown" store result score #n ra.set.tmp run data get storage ra:settings global.props."liquid_drain"."cooldown"
...
execute store result storage ra:settings global.props."liquid_drain"."cooldown" int 1 run scoreboard players get #n ra.set.tmp
