...
execute as @e[type=marker,tag=ra.broken,tag=ra.custom_block.unboxer] at @s run summon item ~ ~ ~ {Item:{id:"minecraft:bat_spawn_egg",count:1,components:{"minecraft:item_model":"minecraft:dispenser","minecraft:item_name":"Unboxer","minecraft:lore":[{text:"Put crates inside; unboxes them into the block in front",color:"gray",italic:false}],"minecraft:custom_data":{ra:{unboxer:1b}},"minecraft:entity_data":{id:"minecraft:bat",Tags:["ra","ra.spawned","ra.place.unboxer"],Silent:1b,NoAI:1b,Invulnerable:1b}}}}
