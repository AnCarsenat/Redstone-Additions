# /ra_wires:blocks/electric_wire/goggles
...
data modify storage ra:temp block_name set value "Wire"
...
# "Enabled: on" was true of every wire ever placed. What a wire can usefully tell
# you is what the grid it belongs to is doing.
function ra:tools/goggles/billboard/data_line {path:"available_eu",label:"EU: ",color:"aqua",suffix:" EU",y:0.8}
function ra:tools/goggles/billboard/data_line {path:"grid",label:"",color:"yellow",suffix:"",y:0.55}
function ra:tools/goggles/billboard/data_line {path:"trend",label:"Change: ",color:"gray",suffix:" EU/s",y:0.3}
