...
execute as @e[type=marker,tag=ra.custom_block.ender_power_vault] unless data entity @s data.properties{enabled:0b} unless data entity @s data.properties{mode:"send"} run tag @s add ra.ender.recv_power
