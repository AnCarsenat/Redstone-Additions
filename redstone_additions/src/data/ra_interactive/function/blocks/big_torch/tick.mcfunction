...
execute as @e[type=marker,tag=ra.broken,tag=ra.custom_block.big_torch] at @s run summon item ~ ~ ~ {Item:{id:"minecraft:bat_spawn_egg",count:1,components:{"minecraft:item_model":"minecraft:torch","minecraft:item_name":'Big Torch',"minecraft:custom_data":{ra:{big_torch:1b}},"minecraft:entity_data":{id:"minecraft:bat",Tags:["ra","ra.spawned","ra.place.big_torch"],Silent:1b,NoAI:1b,Invulnerable:1b}}}}
