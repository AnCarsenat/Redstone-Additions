...
execute as @e[type=marker,tag=ra.custom_block.breeder] at @s unless block ~ ~ ~ dispenser run tag @s add ra.broken
execute as @e[type=marker,tag=ra.broken,tag=ra.custom_block.breeder] at @s run kill @e[type=item,nbt={Item:{id:"minecraft:dispenser"}},distance=..2,limit=1,sort=nearest]
...
execute as @e[type=marker,tag=ra.custom_block.breeder] at @s if block ~ ~ ~ dispenser[triggered=true] run execute if data block ~ ~ ~ Items[0] positioned ^ ^ ^1 run function ra_interactive:blocks/breeder/try_breed
