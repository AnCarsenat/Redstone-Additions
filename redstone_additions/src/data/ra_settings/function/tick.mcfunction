...
# Server-settings sessions: count them down, end the quiet ones, run the clicks.
execute as @a[tag=ra.admin] run scoreboard players remove @s ra.admin.ttl 1
execute as @a[tag=ra.admin,scores={ra.admin.ttl=..0}] run function ra_settings:admin_expire
