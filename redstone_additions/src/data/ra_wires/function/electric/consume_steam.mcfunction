...
execute unless data storage ra:transport cur{medium:"steam"} run return 0
execute unless score #net_amount ra.tr.tmp matches 20.. run return 0

execute store result score #steam_used ra.wires.tmp run function ra_lib:transport/net/take {amount:20}
