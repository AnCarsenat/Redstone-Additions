# /ra_lib:input/backend/writable_book/restore_offhand_from_tmp
...
$execute unless data storage ra:input tmp.item{id:"minecraft:air"} store success score @s ra.temp run item replace entity @s weapon.offhand from entity @e[type=minecraft:armor_stand,tag=ra.input.restore_tmp_new,limit=1,sort=nearest] weapon.mainhand
